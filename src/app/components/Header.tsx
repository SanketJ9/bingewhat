...
export default function Header() {
  ...
  return (
    <header className="absolute top-0 left-0 z-100 w-[100%] text-white">
          ...
          <div className="header-search-profile flex justify-end items-center gap-4 flex-1">
            <form className="max-w-md w-full ">   
                <div className="relative rounded-lg overflow-hidden border-[#3cd293] border-2 bg-[#0b3546] text-[#3cd293] flex items-center">
                    <input type="search" id="default-search" className="block w-full py-2 px-4 text-sm text-gray-200 border rounded-lg border-none outline-none " placeholder="Search Movies, Tv Series..." required />
                    <button className="absolute top-[50%] right-[4px] translate-y-[-50%] px-2 w-auto h-[80%] bg-[#0f455c] text-[#3cd293] flex justify-center items-center rounded-md cursor-pointer hover:bg-[#3cd293] hover:text-[#0f455c] transition-all ease-in duration-300"><IoSearch className="inline-block text-sm "/></button>
                </div>
            </form>
      ...
      {/* Mobile Menu */}
      {isOpen && (
        <nav className="lg:hidden bg-background absolute top-0 left-0 h-[100vh] w-[100vw] z-60 b px-4 flex flex-col gap-6">
          <div className="h-16 flex items-center">
            <Link href="/" className=""  onClick={() => setIsOpen(!isOpen)}>
            <div className="relative w-[120px] md:w-[180px] lg:w-[200px] h-[30px] md:h-[40px] lg:h-[50px]">
              <Image
                src="/logo.png"    
                alt="Logo"
                fill
